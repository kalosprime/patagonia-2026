...
export const GearChecklist = () => {
  ...
  const addItem = (catId: number) => { 
    ...
    if (!text) return; 
    setCategories(categories.map((cat) => cat.id === catId ? { ...cat, items: [...cat.items, text] } : cat)); 
    setNewItemText({ ...newItemText, [catId]: '' }); 
  };
  const removeItem = (catId: number, itemToRemove: string) => { 
    setCategories(categories.map((cat) => cat.id === catId ? { ...cat, items: cat.items.filter((i: string) => i !== itemToRemove) } : cat)); 
    setChecked(checked.filter((i: string) => i !== itemToRemove)); 
  };
