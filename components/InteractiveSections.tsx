...
export const Highlights = () => {
  const data = [
    { title: 'Hito Año Nuevo', desc: 'Festejo en Lago Falkner. Llegada el 28/12.', icon: <Star size={16} />, color: 'bg-yellow-50', link: 'https://www.google.com/maps/search/Lago+Falkner+Neuquen/' },
...
export const Itinerary = () => {
  ...
  return (
    <div className="space-y-4">
      {saving && <div className="fixed top-4 right-4 bg-slate-900 text-white px-4 py-2 rounded-full text-[10px] font-black z-50 flex items-center gap-2 shadow-2xl animate-bounce"><RefreshCw size={10} className="animate-spin" /> SINCRONIZANDO...</div>}
      {items.map((i) => (
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white p-6 rounded-[2.5rem] shadow-sm space-y-3">
        ...
        <button onClick={() => { if(name && text) { const n = [{id:Date.now(), name, text, category:'aviso' as const, date:new Date().toLocaleDateString()}, ...notes]; save(n); setText(''); } }} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold text-xs uppercase tracking-widest">Publicar</button>
      </div>
      {notes.map(n => (
...
export const DiscardedPlaces = () => {
  const data = [
    { n: 'Playa Catritre', r: 'Ambiente familiar. Playa Yuco es mejor.', t: 'San Martín' },
    { n: 'Laguna Negra', r: 'Camping medio pelo.', t: 'Bariloche' },
    { n: 'Cascada Vullignanco', r: 'No destaca tanto.', t: 'Ruta 40' },
    { n: 'El Bolsón', r: 'Suma demasiado manejo.', t: 'Sur' }
  ];
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-slate-900 text-left">
      {data.map((p, i) => (
        <div key={i} className="bg-slate-50 p-5 rounded-2xl flex justify-between items-center opacity-60">
