...
export const Itinerary = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('v5_itinerary');
    if (saved) setItems(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem('v5_itinerary', JSON.stringify(items));
  }, [items]);
...
export const CrewNotes = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('v5_notes');
...
export const GearChecklist = () => {
  ...
  const [categories, setCategories] = useState<any[]>(initial);
  const [checked, setChecked] = useState<string[]>([]);
  const [newItemText, setNewItemText] = useState<{ [key: number]: string }>({});

  useEffect(() => {
    const sCat = localStorage.getItem('v5_gear');
    const sCheck = localStorage.getItem('v5_checked');
    if (sCat) setCategories(JSON.parse(sCat));
    if (sCheck) setChecked(JSON.parse(sCheck));
  }, []);

  useEffect(() => {
    localStorage.setItem('v5_gear', JSON.stringify(categories));
    localStorage.setItem('v5_checked', JSON.stringify(checked));
  }, [categories, checked]);
  ...
  const removeItem = (catId: number, itemToRemove: string) => { setCategories(categories.map(cat => cat.id === catId ? { ...cat, items: cat.items.filter((i: string) => i !== itemToRemove) } : cat)); setChecked(checked.filter((i: string) => i !== itemToRemove)); };
